...
@Component({
  selector: 'app-law-change-comments-bar-chart',
  templateUrl: './law-change-comments-bar-chart.component.html',
  styleUrls: ['./law-change-comments-bar-chart.component.css']
})
export class LawChangeCommentsBarChartComponent {
  users: User[]=[];
  law:LawChange;
    ...
    if (this.data != null && this.data != []&&this.comments!=null) {
      ...
      for (let i = 0; i < this.comments.length; i++) {
        ...
        if(this.comments[i].kommentare_likes.length>0) {
          if (this.comments[i].typ == 'v' && this.comments[i].kommentar_bezugs_id == this.law.gesetzes_id) {
            countdislike=0;
            countlike=0;
      ...
      this.datas.datasets.push({label: 'Likes', backgroundColor: '#e9300b', borderColor: '#e9300b', data: like});
      this.datas.datasets.push({label: 'Dislikes', backgroundColor: '#21568c', borderColor: '#21568c', data: dislike});
