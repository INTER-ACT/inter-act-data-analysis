...
@Component({
  selector: 'app-passive-line-chart',
  templateUrl: './passive-line-chart.component.html',
  styleUrls: ['./passive-line-chart.component.css']
})
export class PassiveLineChartComponent {
  users: User[]=[];
  ...
  generate(){
    ...
    if(this.chart==null) {
      this.chart = new Chart('linechartpassive', {
        type: 'line',
        labels: [],
        data: {
          datasets: []
      ...
      for (let i = 0; i < this.users.length; i++) {
        ...
        for (let y = 0; y < this.users[i].aktivitaeten.length; y++) {
          if((this.users[i].aktivitaeten[y].datum.toString()>startDate|| this.startDate==null)&& (this.users[i].aktivitaeten[y].datum.toString()<endDate||this.endDate==null) ) {
              ...
              if(this.users[i].aktivitaeten[y].aktivitaet=='rating'||this.users[i].aktivitaeten[y].aktivitaet=='comment_like')
              {
                this.count++;
