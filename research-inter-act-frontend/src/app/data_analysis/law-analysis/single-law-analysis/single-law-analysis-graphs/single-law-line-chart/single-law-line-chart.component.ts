...
@Component({
  selector: 'app-single-law-line-chart',
  templateUrl: './single-law-line-chart.component.html',
  styleUrls: ['./single-law-line-chart.component.css']
})
export class SingleLawLineChartComponent  {
  ...
  {
    if(value!= null) {
      if(this.law==null||value.gesetzes_id!==this.law.gesetzes_id) {
        this.law = value;
        this.updateChart();
      }
    }
  ...
  updateChart() {
    ...
    if(this.searchUsers!=null&&this.lawChanges!=null&&this.ratings!=null&&this.comments!=null) {
      for (let i = 0; i < this.lawChanges.length; i++) {
          let selectedUser=false;
          if(this.lawChanges[i].aenderungs_bezugs_id==this.law.gesetzes_id&&this.lawChanges[i].typ=='g') {
      ...
      this.datas.push({label: 'Änderungen', fill: null, borderColor: '#0ed8cd', data: this.data});
      ...
      for (let i = 0; i < this.comments.length; i++) {
        let selectedUser=false;
        if(this.comments[i].kommentar_bezugs_id==this.law.gesetzes_id&&this.comments[i].typ=='g') {
          for (let y = 0; y < this.searchUsers.length; y++) {
      ...
      this.datas.push({label: 'Kommentare', fill: null, borderColor: '#e9300b', data: this.data});

      this.count=0;
      this.data=[];
      for (let i = 0; i < this.ratings.length; i++) {
        let selectedUser = false;
        if (this.ratings[i].rating_bezugs_id == this.law.gesetzes_id && this.ratings[i].typ == 'g') {
          for (let y = 0; y < this.searchUsers.length; y++) {
      ...
      this.datas.push({label: 'Bewertungen', fill: null, borderColor: '#21568c', data: this.data});
      ...
      this.datas.push({label: 'Alles', fill: null, borderColor: '#7be00f', data: this.alldata});
